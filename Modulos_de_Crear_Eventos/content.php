...
<?php
    ...
    function event(){
        ...
        $html .= "<h2 style ='colo'> <a style='color: white;' href=\"../Mis_eventos.php\" class=\"btn btn-default\">Regresar</a></h2>";
        $html .= "</legend>"; 
        $html .= "<legend class='text-center header'>";
        ...
        $data = $cn->prepare("SELECT * FROM categoria"); 
        $data->execute();

        while($datos = $data->fetch(PDO::FETCH_ASSOC)){
            $html .= "<option value='".$datos["idCategoria"]."'>".$datos["Categoria"]."</option>";
        }
